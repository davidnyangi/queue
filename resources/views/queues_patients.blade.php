                                    ...
                                    <tbody>
                                            @if($todaysQueue)
                                                ...
                                                <tr>
                                                    ...
                                                    <!-- <td id="queuenumber">
                                                        @if(strpos($todayQueue->Department, 'PRV.OPT') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$popt ?>
                                                            {{substr($todayQueue->Department,4,3).'-'.$popt}}
                                                            <?php $popt++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'STD.OPT') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$sopt ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$sopt}}
                                                        <?php $sopt++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'STD.RHB.PO') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$srpo ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$srpo}}
                                                        <?php $srpo++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'PRV.RHB.PO') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$prpo ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$prpo}}
                                                        <?php $prpo++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'STD.RHB.PHY') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$srp ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$srp}}
                                                        <?php $srp++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'STD.VVF') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$sVV ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$sVV}}
                                                        <?php $sVV++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'PRV.VVF') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$pVV ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$pVV}}
                                                        <?php $pVV++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'PRV.GYN') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$pgyn ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$pgyn}}
                                                        <?php $pgyn++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'STD.GYN') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$sgyn ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$sgyn}}
                                                        <?php $sgyn++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'PRV.RHB.PHY') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$prp ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$prp}}
                                                        <?php $prp++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'STD.OTH') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$b ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$b}}
                                                        <?php $b++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'PRV.OTH') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$po ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$po}}
                                                        <?php $po++;  ?>
                                                         @elseif(strpos($todayQueue->Department, 'STD.ENT') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$sent ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$sent}}
                                                        <?php $sent++;  ?>
                                                         @elseif(strpos($todayQueue->Department, 'PRV.ENT') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$e ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$e}}
                                                        <?php $e++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'PRV.EYE') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$c ?> 
                                                        {{substr($todayQueue->Department,4,3).'-'.$c}}
                                                        <?php $c++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'STD.EYE') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$se ?>
                                                        {{substr($todayQueue->Department,4,3).'-'.$se}}
                                                        <?php $se++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'STD.SUN') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$sImaging ?>
                                                        {{'IMAGING -'.$sImaging}}
                                                        <?php $sImaging++;  ?>
                                                        @elseif(strpos($todayQueue->Department, 'PRV.SUN') !== false) 
                                                        <?php $dept = substr($todayQueue->Department,4,3).'-'.$pImaging ?>
                                                        {{'IMAGING -'.$pImaging}}
                                                        <?php $pImaging++;  ?>
                                                        @endif
                                                    </td> -->
                                                   ...
                                                   <!--  <td id="patientstatus"><span class="badge badge-secondary" style="background:purple;boarder-radius:2px;">To be Called</span></td> -->
                                                    ...
                                                    <!-- <button id="printQueue" type="button" class="btn btn-primary waves-effect"  data-patientname="{{ $todayQueue->firstname}} &nbsp; {{ $todayQueue->lastname}}" data-dept="{{$deptname}}" data-queue="{{$dept}}" data-patientip="{{ $todayQueue->Encounter_ip}}" data-starttime="{{ $todayQueue->Timea}}" data-waitingtime="{{ $todayQueue->Wait}}" >PRINT</button> -->
                                                    </td>
                                                </tr>
                                                @endforeach
                                            @else
