...
// convert(date,oc_encounter_begindate) = CONVERT(date, getdate()) and OC_ENCOUNTER_ENDDATE is null order by wait asc');
		 	$todaysQueue = DB::connection('sqlsrv')->select('select firstname,lastname,OC_QUEUE_BEGIN,OC_QUEUE_SUBJECTUID,OC_QUEUE_PATIENTNUMBER from OC_TODAYSQUEUES,AdminView a where 
OC_PATIENT_CALLEDSTATUS =\'seen\' and CONVERT(date,OC_QUEUE_BEGIN) = CONVERT(date,getdate())
and OC_QUEUE_SUBJECTUID = a.personid');
